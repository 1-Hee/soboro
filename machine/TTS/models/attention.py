...
def _location_score(processed_query, keys, normalize, location_aware):
  ...
  if normalize:
    ...
    b = variable_scope.get_variable(
        "attention_b", [num_units], dtype=dtype,
        initializer=init_ops.zeros_initializer())
    return math_ops.reduce_sum(v * math_ops.tanh(keys + processed_query + location_aware + b), [2])


def smoothing(x):
    return tf.nn.sigmoid(x) / tf.reduce_sum(tf.nn.sigmoid(x), axis=-1, keepdims=True)


def location_layer(x, num_units):
    x = tf.expand_dims(x, 2)
    x = tf.layers.conv1d(x, filters=32, kernel_size=32, padding='same', use_bias=False,
                         kernel_initializer=xavier_initializer())
    x = tf.layers.dense(x, units=num_units, use_bias=False, kernel_initializer=xavier_initializer())
    return x
...
class LocationSensitiveAttention(_BaseAttentionMechanism):
  def __init__(self,
    ...
    if probability_fn is None:
      probability_fn = smoothing
    if dtype is None:
      dtype = dtypes.float32
    wrapped_probability_fn = lambda score, _: probability_fn(score)
    super(LocationSensitiveAttention, self).__init__(
        query_layer=layers_core.Dense(
            num_units, name="query_layer", use_bias=False, dtype=dtype, kernel_initializer=xavier_initializer()),
        memory_layer=layers_core.Dense(
            num_units, name="memory_layer", use_bias=False, dtype=dtype, kernel_initializer=xavier_initializer()),
        memory=memory,
    ...
    self._name = name
    self._location_layer = location_layer


  def __call__(self, query, state):
    with variable_scope.variable_scope(None, "bahdanau_attention", [query]):
      processed_query = self.query_layer(query) if self.query_layer else query
      location_aware = self._location_layer(state, self._num_units)
      score = _location_score(processed_query, self._keys, self._normalize, location_aware)
